...
import { ChangeEvent, DragEvent, PointerEvent as ReactPointerEvent, WheelEvent as ReactWheelEvent, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
export default function HairMotionViewer() {
  ...
  const [camera, setCamera] = useState({ zoom: 1, x: 0, y: 0 }), [panning, setPanning] = useState(false);
  const canvasRef = useRef<HTMLCanvasElement>(null), stageRef = useRef<HTMLDivElement>(null), rafRef = useRef(0), clockRef = useRef(0);
  const panRef = useRef<{ id: number; startX: number; startY: number; x: number; y: number } | null>(null);
  ...
  const draw = useCallback((renderTime = time) => {
    ...
    const baseScale = Math.min((w - pad * 2) / (bounds.maxX - bounds.minX), (h - pad * 2) / (bounds.maxY - bounds.minY)), scale = baseScale * camera.zoom, ox = (w - (bounds.maxX - bounds.minX) * baseScale) / 2 + camera.x, oy = h - (h - (bounds.maxY - bounds.minY) * baseScale) / 2 + camera.y;
    const map = (p: { x: number; y: number }) => ({ x: ox + (p.x - bounds.minX) * scale, y: oy - (p.y - bounds.minY) * scale });
  ...
  }, [viewData, bounds, time, history, lines, hovered, markerColors, camera]);
  ...
  const hitMarker = (event: ReactPointerEvent<HTMLCanvasElement>) => {
    ...
    const rect = canvasRef.current.getBoundingClientRect(), w = rect.width, h = rect.height, pad = 50, baseScale = Math.min((w - pad * 2) / (bounds.maxX - bounds.minX), (h - pad * 2) / (bounds.maxY - bounds.minY)), scale = baseScale * camera.zoom, ox = (w - (bounds.maxX - bounds.minX) * baseScale) / 2 + camera.x, oy = h - (h - (bounds.maxY - bounds.minY) * baseScale) / 2 + camera.y, frame = viewData.frames[Math.min(viewData.frames.length - 1, Math.round(time * 30))];
    const found = frame.points.findIndex((p) => p && Math.hypot(event.clientX - rect.left - (ox + (p.x - bounds.minX) * scale), event.clientY - rect.top - (oy - (p.y - bounds.minY) * scale)) < 18); setHovered(found >= 0 ? found : null);
  };
  const beginPan = (event: ReactPointerEvent<HTMLCanvasElement>) => { if (event.button !== 0) return; event.currentTarget.setPointerCapture(event.pointerId); panRef.current = { id: event.pointerId, startX: event.clientX, startY: event.clientY, x: camera.x, y: camera.y }; setPanning(true); setHovered(null); };
  const moveCanvas = (event: ReactPointerEvent<HTMLCanvasElement>) => { const pan = panRef.current; if (pan?.id === event.pointerId) { setCamera((current) => ({ ...current, x: pan.x + event.clientX - pan.startX, y: pan.y + event.clientY - pan.startY })); return; } hitMarker(event); };
  const endPan = (event: ReactPointerEvent<HTMLCanvasElement>) => { if (panRef.current?.id === event.pointerId) { panRef.current = null; setPanning(false); } };
  const zoomCanvas = (event: ReactWheelEvent<HTMLCanvasElement>) => { event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(), px = event.clientX - rect.left, py = event.clientY - rect.top, baseScale = Math.min((rect.width - 100) / (bounds.maxX - bounds.minX), (rect.height - 100) / (bounds.maxY - bounds.minY)); setCamera((current) => { const zoom = Math.min(8, Math.max(.35, current.zoom * Math.exp(-event.deltaY * .001))); const ratio = zoom / current.zoom, ox = (rect.width - (bounds.maxX - bounds.minX) * baseScale) / 2, oy = rect.height - (rect.height - (bounds.maxY - bounds.minY) * baseScale) / 2; return { zoom, x: px - ox - (px - ox - current.x) * ratio, y: py - oy - (py - oy - current.y) * ratio }; }); };
  ...
  return <main className="app-shell" onDragOver={(e) => e.preventDefault()} onDrop={drop}>
    <header className="topbar"><div><h1>Hair Motion Viewer</h1></div><label className="file-button"><input type="file" accept=".csv,text/csv" onChange={openFile} />CSVを開く</label></header>
    ...
    </aside><section className="viewer-panel"><div className="viewer-head"><div><span className="live-dot" /> MOTION SPACE <small>XY · mm · drag: move · wheel: zoom</small></div><div className="view-tools"><button onClick={() => setCamera({ zoom: 1, x: 0, y: 0 })}>RESET VIEW</button><span>DISPLAY 30 Hz</span></div></div>
      <div className="stage" ref={stageRef}><canvas className={panning ? "panning" : ""} ref={canvasRef} onPointerDown={beginPan} onPointerMove={moveCanvas} onPointerUp={endPan} onPointerCancel={endPan} onPointerLeave={(event) => { if (!panning) setHovered(null); endPan(event); }} onWheel={zoomCanvas} />{!data && !error && <div className="empty">Loading sample data…</div>}{error && <div className="empty error">{error}<small>CSVをここへドロップしてください</small></div>}{hovered !== null && currentPoint && <div className="tooltip"><b>{MARKERS[hovered].name}</b><span>Time <em>{time.toFixed(2)} s</em></span><span>X <em>{currentPoint.x.toFixed(2)} mm</em></span><span>Y <em>{currentPoint.y.toFixed(2)} mm</em></span><span>Scalp position <em>{MARKERS[hovered].position} / 100</em></span></div>}</div>
